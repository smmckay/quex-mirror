...
verify_content(QUEX_NAME(Buffer)* me)
{
    ...
    hwut_verify(me->begin(me)[0]             == QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
    hwut_verify(me->content_space_end(me)[0] == QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
    hwut_verify(me->content_end(me)[0]       == QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
             ...
             QUEX_TYPE_LEXATOM_EXT* (position_register[5]))
{
    ...
    me->read_m1             = buffer->_read_p == buffer->_memory._front ? 
                              QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER : buffer->_read_p[-1];
    me->lexeme_start_p      = buffer->_lexeme_start_p;
    me->lexeme_start_m1     = buffer->_lexeme_start_p == buffer->_memory._front ? 
                              QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER : buffer->_lexeme_start_p[-1];
                         ...
                         bool               ConverterF)
{
    ...
    if( Verdict == E_LoadResult_FAILURE ) {
        ...
        if(    buffer->_lexeme_start_p > &buffer->_memory._front[1] 
            && me->lexeme_start_m1 != QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER ) {
    ...
    if(    buffer->_read_p > &buffer->_memory._front[1] 
        && me->read_m1 != QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER ) {
    ...
    hwut_verify(buffer->input.end_p[0] == QUEX_TestAnalyzer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
