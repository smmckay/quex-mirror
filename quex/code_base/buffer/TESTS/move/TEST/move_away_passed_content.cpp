 ...
 *
 * Let this file be compiled with '-DQUEX_UT_SETTING_BUFFER_FALLBACK_N_EXT=3'
 * for all experiments. Multiple versions of compiled objects may exist:
 *
 *        QUEX_TYPE_LEXATOM_EXT  QUEX_UT_SETTING_BUFFER_FALLBACK_N_EXT
 *          uint8_t             0
 *          uint8_t             1
 *          uint8_t             2
 *          uint32_t            2
...
main(int argc, char** argv)
{
    ...
    while( G_next(&it) ) {
        ...
        if( ! end_of_stream_in_buffer_f && (min_p - &buffer._memory._front[1] >= QUEX_UT_SETTING_BUFFER_FALLBACK_N_EXT) ) {
            /* The move is only restricted by '_read_p', '_lexeme_start_p' and 
             * the fallback region size. Thus, ...                           */
            hwut_verify(   (buffer._read_p         - &buffer._memory._front[1] == QUEX_UT_SETTING_BUFFER_FALLBACK_N_EXT)
                        || (buffer._lexeme_start_p - &buffer._memory._front[1] == QUEX_UT_SETTING_BUFFER_FALLBACK_N_EXT));
        }
        else {
