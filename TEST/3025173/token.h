...
public:
    ...
    { number_ = number; }
    std::basic_string<uint8_t> get_text() const
    { return text_; }
    void                                   set_text(std::basic_string<uint8_t>& text)
    { text_ = text; }
    ...
    void set(const QUEX_TYPE_TOKEN_ID ID, const std::basic_string<uint8_t>& Value0)
    { id = ID; text_ = Value0; }
    void set(const QUEX_TYPE_TOKEN_ID ID, int Value0, const std::basic_string<uint8_t>& Value1)
...
inline void Token_copy(Token* me, const Token* Other) {
  *me = *Other;
}
